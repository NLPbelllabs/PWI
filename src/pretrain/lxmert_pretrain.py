...
def get_tuple_multilingual_hybrid_pretrain(splits: str, bs: int, shuffle=False, drop_last=False, num_workers=0, topk=-1, image_only_splits=None, text_only_splits = None, limit_source = [], restrict_source = None) -> DataTuple:
    # Decide which QA datasets would be used in pre-training.
    # Options: vqa, gqa, visual7w
    # Note: visual7w is a part of vgqa, we take the name here.
    qa_sets = args.qa_sets
    ...
    if text_only_splits is not None:
        ...
        for split in text_only_splits.split("+"):
            if not("book_corpus_dewiki" in split or "sbu" in split):
                print('COCO captions')
            ...
            elif "book_corpus_dewiki" in split:
                text_only_dataset = None
                text_only_file_all = args.text_only_de_file + '+' + args.book_corpus_path
                print(text_only_file_all.split('+'))
                text_only_dataset_torch = GeneralCorpusNP2(ann_file=text_only_file_all, 
                                                            pretrained_model_name=args.huggingface+args.tokenizer, 
                                                            tokenizer=None, 
                                                            seq_len=args.get("text_only_max_seq_len", 64), 
                                                            min_seq_len=args.get("text_only_min_seq_len", 64), 
                                                            encoding="utf-8", 
                                                            on_memory=True)
                datasets_list.append(text_only_dataset)
                datasets_list_torch.append(text_only_dataset_torch)
        ...
        for split in text_only_splits.split("+"):
            if not("book_corpus" in split or "sbu" in split):
                ...
                if "book_corpus" in split and args.get("text_shared_memory", False):
                    ...
                    pass
                text_only_dataset_torch = text_class(ann_file=args.book_corpus_path if "book_corpus" in split else args.sbu_path, 
                                                    pretrained_model_name=args.huggingface+"bert-base-uncased", 
                                                    tokenizer=None, 
                                                    seq_len=args.get("text_only_max_seq_len", 64), 
                                                    min_seq_len=args.get("text_only_min_seq_len", 64), 
                                                    encoding="utf-8", 
...
from src.pretrain.lxmert_data import symbolic_vocab
...
class LXMERT:
    def __init__(self, max_seq_length):
        ...
        # Build model
        self.model = LXRTPretraining.from_pretrained(args.huggingface+args.tokenizer,
            args = args,
    ...
    def train(self, train_tuple: DataTuple, eval_tuple: DataTuple):
        ...
        for epoch in range(args.epochs):
            ...
            if avg_eval_loss < best_eval_loss:
                ...
                self.save("BEST_EVAL_LOSS_%s" % (args.get("lang",'en')))
            self.save("Epoch%02d_%s" % (epoch+1,args.get("lang",'en')))
