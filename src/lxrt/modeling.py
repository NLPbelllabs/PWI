...
class VisualConfig(object):
    ...
    def __init__(self,
        ...
        self.symbolic_embedding_number = self.obj_id_num*len(args.objects_vocab)+self.attr_id_num*len(args.attributes_vocab)+3

        print('self.symbolic_embedding_number:',self.symbolic_embedding_number)
...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        symbolic_embedding_number = 1600*len(args.objects_vocab)+400*len(args.attributes_vocab)+3
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (sys.version_info[0] == 2
            ...
            self.symbolic_embedding_number = symbolic_embedding_number
...
class BertAttOutput(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor):
        #print('BertAttOutput:',len(hidden_states))
        #print('BertAttOutput:',hidden_states[0].shape,hidden_states[1].shape)
...
class BertSelfattLayer(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        ...
        if args.get("output_attention", False):
            ...
            #print('BertSelfattLayer attention_probs:',len(attention_probs),attention_probs[0].shape)
...
class BertEmbeddingsWithVisualEmbedding(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.symbolic_embedding = nn.Embedding(config.symbolic_embedding_number, config.hidden_size)
    ...
    def forward(self,
        ...
        if not args.get("hybrid_embedding", False):
            if args.get("symbolic", False):
                ...
                visn_feats = None
        # print('self.multi_choice: ',self.multi_choice)
        if self.multi_choice != 0:
            ...
            #print('visn_feats.shape:',visn_feats.shape)
        ...
        if self.visualbert_style:
            ...
            for layer_module in self.layer:
                #print(layer_module)
                #print('LXRTEncoder: joint_feats+joint_mask',joint_feats.shape,joint_mask.shape)
                if args.get("output_attention", False):
                    #print(self.layer)
                    joint_feats, attention_weights = layer_module(joint_feats, joint_mask)
                    #joint_feats = layer_module(joint_feats, joint_mask)
                    #print('LXRTEncoder: joint_feats',joint_feats.shape)
                    all_attention_weights.append(attention_weights)
                else:
                    joint_feats = layer_module(joint_feats, joint_mask)
                #print('LXRTEncoder: joint_feats',joint_feats.shape)
                #print('LXRTEncoder: lang_feats',lang_feats.shape)
                #print('LXRTEncoder: visn_feats',visn_feats.shape)
            if args.get("additional_attention_layer", False):
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        #print('LXRTModel: embedding_output',embedding_output.shape)
        #print('LXRTModel: concated_mask',concated_mask.shape)
        # self.encoder will not distinguish between visual inputs, visual tag inputs or text inputs
        if args.get("output_attention", False):
            #print('LXRTModel embedding_output',embedding_output.shape)
            combined_feats, attention_weights = self.encoder(
            embedding_output,
            concated_mask,
            visn_feats=None,
            visn_attention_mask=None)
            combined_feats=combined_feats[0]
            #print('LXRTModel combined_feats',len(combined_feats),combined_feats.shape)
            #print('LXRTModel attention_weights', len(attention_weights),attention_weights[0].shape)
            #print('LXRTModel _',_)
        else:
            combined_feats, _ = self.encoder(
                ...
                visn_attention_mask=None)
            #print('LXRTModel _',_)
            #print('LXRTModel combined_feats',len(combined_feats),combined_feats[0].shape)
            #print('LXRTModel combined_feats',len(combined_feats),combined_feats[0].shape)
        if attention_mask is not None:
    ...
    def forward(self,
        ...
        if obj_labels is not None and self.task_obj_predict and not args.get("disable_visual_and_tag_objective", False):
            ...
            for key in self.args.visual_losses.split(","):
                ...
                if key == "attr" or key == "obj":
                    '''modified for multilingual training'''
                    if key == "obj":
                        label = label%1600
                    elif key == "attr":
                        label = label%400
        ...
        if visual_tags_objective is not None and not args.get("disable_visual_and_tag_objective", False):
            ...
            if args.get("use_bert_input_for_tags", False):
                ...
                #print('visual_tags_objective_1:',tags_output.view(-1, self.config.vocab_size).shape)
                #print('visual_tags_objective_2:',visual_tags_objective.view(-1).shape)
...
class LXRTFeatureExtractionAttention(BertPreTrainedModel):
    """
    BERT model for classification.
    """
    def __init__(self, config, mode='lxr'):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_attention_mask=None, return_both = False, visual_feats_seg_ids = None):
        feat_seq, pooled_output, attention_weights = self.bert(input_ids, token_type_ids, attention_mask,
                                            visual_feats=visual_feats,
                                            visual_attention_mask=visual_attention_mask,
                                            visual_feats_seg_ids = visual_feats_seg_ids)
        print('feat_seq, pooled_output, attention_weights')
        print(feat_seq.shape, pooled_output.shape, attention_weights.shape)
        if return_both:
            return feat_seq, pooled_output, attention_weights
        ...
        if 'x' == self.mode:
            ...
            return feat_seq, attention_weights