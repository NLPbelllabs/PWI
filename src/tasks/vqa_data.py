...
class VQATorchDataset(Dataset):
    def __init__(self, dataset: VQADataset, args):
        ...
        if dataset.splits==['minival_food']:
            dataset.splits=['minival']
        #elif dataset.splits==['train_food'] or dataset.splits==['train_food_v2'] or dataset.splits==['train_food_v3']  or dataset.splits==['train_animal_v3'] :
        elif dataset.splits[0].startswith('train'):
            dataset.splits=['train']
        elif dataset.splits[0].startswith('nominival'):
        #elif dataset.splits==['nominival_food_v2'] or dataset.splits==['nominival_food_v3'] or dataset.splits==['nominival_animal_v3']:
            dataset.splits=['nominival']
        ...
        if self.limit_to_symbolic_split:
            ...
            print('self.raw_dataset.name',self.raw_dataset.name)
            self.vg_gqa = vg_gqa(None, split = "val" if self.raw_dataset.name in ["minival","minival_food"]  else "train", transforms=None, num_im=-1)
    ...
    def __getitem__(self, item: int):
        ...
        if self.custom_coco_data:
            ...
            if len(args.get('objects_vocab', False)) > 1 and len(args.get('attributes_vocab', False)) > 1 and args.get('german_train', True):
                prob = random.random()
                #print('German and English in VQA')
                #print('prob:',prob)
                if prob > 0.5:
                    obj_labels=[i+1600 for i in obj_labels]
                    attr_labels=[i+400 for i in attr_labels]
            elif len(args.get('objects_vocab', False)) > 1 and len(args.get('attributes_vocab', False)) > 1 and args.get('german_only', True):
                #print('German only in VQA')
                obj_labels=[i+1600 for i in obj_labels]
                attr_labels=[i+400 for i in attr_labels]
        ...
        if 'label' in datum:
            ...
            multi_ans=list(label.keys())
            for ans, score in label.items():
                target[self.raw_dataset.ans2label[ans]] = score
            return ques_id, feats, boxes, ques, tags, target,obj_labels, obj_confs, attr_labels, attr_confs,img_id,ques_type,multi_ans
        else:
